...
import {
  ...
  SoftConstraintsBasedCostCalculator,
  CostLimitCalculator,
} from ".";
...
export class CostCalculator {
  ...
  constructor(data: CostCalculatorData) {
    this.softConstraintsCostCalculatorData =
      data.softConstraintsCostCalculatorData;
    this.schedulerContextData = data.schedulerContextData;
  }

  calculate(group: CourseGroup) {
    group.cost = new SoftConstraintsBasedCostCalculator(
      this.softConstraintsCostCalculatorData
    ).calculate(
      this.schedulerContextData.currentSchedule,
      this.schedulerContextData.softConstraints
    );
  ...
  private considerDiscardedAverageCostsWithTheirPercentage = (
    group: CourseGroup
  ): number => {
    ...
    variables.forEach((variable) => {
      if (
        variable !== this.schedulerContextData.currentVariable &&
        !variable.hasAssignedValue()
      ) {
    ...
    if (discardedGroups.length !== 0) {
      ...
      if (discardingPercent >= DISCARDING_GROUPS_AVERAGE_COSTS_THRESHOLD) {
        averageCosts =
          discardedGroups.reduce((accumalator, courseGroup) => {
            ...
            const cost = new SoftConstraintsBasedCostCalculator(
              softConstraintsCostCalculatorData
            ).calculate(
              this.schedulerContextData.currentSchedule,
              this.schedulerContextData.softConstraints
    ...
    const cost = group.cost; //+ discardingPercent;
    group.discardedGroupsLength = discardedGroups.length;
    group.averageDiscardedCost = averageCosts;
  ...
  private getProjectedValue = (oldAverageCosts: number, slope = 1): number => {
    const LIMIT = new CostLimitCalculator(
      this.schedulerContextData.softConstraints
    ).calculate();
